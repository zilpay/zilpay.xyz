  ...
  <div :class="b()">
    <div :class="b('container')">
      ...
      <Jumbotron>
        ...
        <Alert>contract Balance: {{ balance }} ZIL</Alert>
        <div :class="b('rol-value')">
          {{ range }}
        </div>
        <Range v-model="range" :max="rangeMax" :min="rangeMin" :class="b('range')" />
        <div :class="b('win-row')">
          <div :class="b('win-amount')">
        ...
        <Button lg :class="b('button')" @click="roll">
          Roll
        </Button>
      </Jumbotron>
    </div>
    <Modal :name="modalInstance.name" :title="modalInstance.title">
      <div :class="b('modal-content')">
        <img width="300" :src="modalInstance.img" :class="b('modal-img')">
      </div>
    </Modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    async getState () {
      ...
      this.balance = utils.units.fromQa(new utils.BN(state._balance), 'zil')
    },
    async roll () {
      const test = this.zilPayTest()
      if (!test) {
    ...
    showResult (value) {
      ...
      const amount = utils.units.fromQa(new utils.BN(value), 'zil')
